...
│ 汇总：范围 / 条件 / 批选    [取消批选]                              │
├────────────────────────────────────────────────────────────────────┤
│ 条件：[ (GBK|CHS|简);(1080p)  ]  GBK|CHS|简  BIG5|CHT|繁  720p     │
│      1080p  1080p+  2160p|4K  HEVC|H265  AVC|H264  字幕|内嵌|外挂   │
│      [条件选择]  [×]            错误提示                            │
├────────────────────────────────────────────────────────────────────┤
│ 搜索：[ 关键词空格分隔 ]  [✓]全局  [✓]含删除                       │
├───────────────┬────────────────────────────────────────────────────┤
...
│  分组：其他   │                                                    │
├───────────────┴────────────────────────────────────────────────────┤
│ 动作栏：立即检查 / 勾选当前筛选 / 取消当前勾选 / 复制勾选 / ...    │
├────────────────────────────────────────────────────────────────────┤
│ 状态栏：正常 | RSS 12 个 | 新增 5 | 已勾选 5 | 下次检查 28 分钟后   │
└────────────────────────────────────────────────────────────────────┘
```
...
> 下方展示程序图标。主窗口为实际运行的 WinForms 界面，包含菜单、汇总栏、条件栏、搜索栏、侧边栏与条目网格（详见上文“主界面布局”）。

<p align="center">
  <img src="Resources/app.png" alt="RSS Magnet Collector 图标" width="160"/>
</p>

> 上图为程序托盘 / 任务栏图标。如需截图主界面，可在本地桌面运行 `RRSMC.exe`，
> 关闭主窗口（最小化到托盘）后左键托盘图标即可呼出主窗口，按 `Win + Shift + S` 等截图工具截取后替换本占位。

---

## 设计原则

- **轻量便携**：单程序 + data 文件夹，整个目录可复制迁移。
- **不引入重依赖**：无数据库、无下载器集成、无远程同步，零外部 NuGet。
- **状态优先**：托盘图标和汇总栏始终反映当前运行/网络/RSS 状态。
- **复制便捷**：剪贴板是默认导出方式，一行一个 magnet，CRLF 换行。
- **规则清晰**：条件表达式刻意简单，`;` AND / `|` OR / `!` NOT，兼顾易用与可组合。
